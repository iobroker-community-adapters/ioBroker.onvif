...
module.exports = function (Cam) {
  ...
  Cam.prototype.getRecordings = function (callback) {
    this._request(
      {
        service: "recording",
        body: this._envelopeHeader() + '<GetRecordings xmlns="http://www.onvif.org/ver10/recording/wsdl"/>' + this._envelopeFooter(),
      },
  ...
  Cam.prototype.getRecordingJobs = function (callback) {
    this._request(
      {
        service: "recording",
        body: this._envelopeHeader() + '<GetRecordingJobs xmlns="http://www.onvif.org/ver10/recording/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
          /**
  ...
  Cam.prototype.createRecordingJob = function (options, callback) {
    ...
    try {
      commandSend =
        ...
        "<Tracks>" +
        "<SourceTag>" +
        ((((options ? options.source : "") ? options.source.tracks : "") ? options.source.tracks.sourceTag : "") || "") +
        "</SourceTag>" +
        "<Destination>" +
        ((((options ? options.source : "") ? options.source.tracks : "") ? options.source.tracks.destination : "") || "") +
        "</Destination>" +
        "</Tracks>" +
        "<Extension>" +
    ...
    this._request(
      {
        service: "recording",
        body: this._envelopeHeader() + commandSend + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
  ...
  Cam.prototype.getRecordingSummary = function (callback) {
    this._request(
      {
        service: "search",
        body: this._envelopeHeader() + '<GetRecordingSummary xmlns="http://www.onvif.org/ver10/search/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
    ...
    this._request(
      {
        service: "recording",
        body:
          this._envelopeHeader() + '<GetServiceCapabilities xmlns="http://www.onvif.org/ver10/recording/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
