...
module.exports = function (Cam) {
  ...
  Cam.prototype.getEventProperties = function (callback) {
    this._request(
      {
        service: "events",
        body: this._envelopeHeader() + '<GetEventProperties xmlns="http://www.onvif.org/ver10/events/wsdl"/>' + this._envelopeFooter(),
      },
  ...
  Cam.prototype.getEventServiceCapabilities = function (callback) {
    this._request(
      {
        service: "events",
        body: this._envelopeHeader() + '<GetServiceCapabilities xmlns="http://www.onvif.org/ver10/events/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, res, xml) {
        if (!err) {
  ...
  Cam.prototype.subscribe = function (options, callback) {
    ...
    sendXml +=
      ...
      "<ConsumerReference><a:Address>" +
      options.url +
      "</a:Address></ConsumerReference>" +
  ...
  Cam.prototype.renew = function (options, callback) {
    ...
    if (!subscriptionId) {
      ...
      sendXml +=
        ...
        urlAddress.href +
        "</a:To>" +
        '<SubscriptionId xmlns="http://www.axis.com/2009/event" a:IsReferenceParameter="true">' +
  ...
  Cam.prototype._eventPull = function () {
    if (this.listeners("event").length && this.events.subscription) {
        ...
        function (err, data, xml) {
          if (!err) {
            if (data.notificationMessage) {
              ...
              data.notificationMessage.forEach(
                function (message) {
                  /**
