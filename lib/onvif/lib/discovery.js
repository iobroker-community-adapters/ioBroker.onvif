 ...
 * @class
 */
const Discovery = Object.create(new events.EventEmitter());
...
Discovery.probe = function (options, callback) {
  ...
  const listener = function (msg, rinfo) {
    parseSOAPString(
      msg.toString(),
      function (err, data, xml, _statusCode) {
  ...
  setTimeout(
    function () {
      ...
      callback(
        ...
        Object.keys(cams).map(function (addr) {
          return cams[addr];
        }),
      );
    }.bind(this),
...
function matchXAddr(xaddrs, address) {
  const ipMatch = xaddrs.filter(function (xaddr) {
