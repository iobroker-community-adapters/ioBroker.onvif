 ...
 * });
 */
const Cam = function (options, callback) {
  ...
  this.getSystemDateAndTime(
    function (err, date, xml) {
      ...
      function callUpstartFunctions() {
        ...
        let count = upstartFunctions.length;
        let errCall = false;

        if (count > 0) {
          upstartFunctions.forEach(
            function (fun) {
              fun.call(
                this,
                function (err) {
      ...
      if (err) {
        if (xml && xml.toLowerCase().includes("sender not authorized")) {
          ...
          this._request(
            {
              body:
                this._envelopeHeader() + '<GetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
            },
            function (err, data, xml) {
              try {
                const systemDateAndTime = data[0]["getSystemDateAndTimeResponse"][0]["systemDateAndTime"][0];
      ...
      body:
        ...
        (options.dateTime !== undefined && options.dateTime instanceof Date
          ? "<UTCDateTime>" +
            ...
            "<Hour>" +
            options.dateTime.getUTCHours() +
            "</Hour>" +
            "<Minute>" +
            options.dateTime.getUTCMinutes() +
            "</Minute>" +
            "<Second>" +
            options.dateTime.getUTCSeconds() +
            "</Second>" +
            "</Time>" +
            '<Date xmlns="http://www.onvif.org/ver10/schema">' +
            "<Year>" +
            options.dateTime.getUTCFullYear() +
            "</Year>" +
            "<Month>" +
            (options.dateTime.getUTCMonth() + 1) +
            "</Month>" +
            "<Day>" +
            options.dateTime.getUTCDate() +
            "</Day>" +
            "</Date>" +
            "</UTCDateTime>"
          : "") +
    ...
    function (err, data, xml) {
      if (err || linerase(data).setSystemDateAndTimeResponse !== "") {
        return callback.call(
          this,
          linerase(data).setSystemDateAndTimeResponse !== "" ? new Error("Wrong `SetSystemDateAndTime` response") : err,
          data,
          xml,
        );
      }
  ...
  this._request(
    ...
    function (err, data, xml) {
      if (!err) {
        ...
        ["PTZ", "media", "imaging", "events", "device"].forEach(
          function (name) {
        ...
        if (this.capabilities.extension) {
          Object.keys(this.capabilities.extension).forEach(
            function (ext) {
        ...
        if (this.uri["replay"] && !this.uri["recording"]) {
          const tempRecorderXaddr = this.uri["replay"].href.replace("replay", "recording");
...
Cam.prototype.getServiceCapabilities = function (callback) {
  this._request(
    {
      body: this._envelopeHeader() + '<GetServiceCapabilities xmlns="http://www.onvif.org/ver10/device/wsdl" />' + this._envelopeFooter(),
    },
    function (err, data, xml) {
      if (!err) {
        data = linerase(data);
...
Cam.prototype.getActiveSources = function () {
  ...
  this.videoSources.forEach(
    function (videoSource, idx) {
      ...
      if (appropriateProfiles.length === 0) {
        if (idx === 0) {
          throw new Error("Unrecognized configuration " + JSON.stringify(this.videoSources) + " " + JSON.stringify(this.profiles));
        } else {
          return;
        }
      }
...
Cam.prototype.getDeviceInformation = function (callback) {
  this._request(
    {
      body: this._envelopeHeader() + '<GetDeviceInformation xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
    },
    function (err, data, xml) {
      if (!err) {
...
Cam.prototype.getHostname = function (callback) {
  this._request(
    {
      body: this._envelopeHeader() + '<GetHostname xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
    },
    function (err, data, xml) {
      if (callback) {
...
Cam.prototype.getScopes = function (callback) {
  this._request(
    {
      body: this._envelopeHeader() + '<GetScopes xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
    },
    function (err, data, xml) {
      if (!err) {
        /**
...
Cam.prototype.systemReboot = function (callback) {
  this._request(
    {
      service: "deviceIO",
      body: this._envelopeHeader() + '<SystemReboot xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
    },
...
Cam.prototype._envelopeHeader = function (openHeader) {
  ...
  if (!(openHeader !== undefined && openHeader)) {
    header += "</s:Header>" + '<s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">';
  }
  return header;
};
