    ...
    function (err, data, xml) {
      ...
      if (err) {
        if (xml && xml.toLowerCase().includes("sender not authorized")) {
          ...
          this._request(
            {
              body: this._envelopeHeader() + '<GetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
            },
            function (err, data, xml) {
              try {
                let systemDateAndTime = data[0]["getSystemDateAndTimeResponse"][0]["systemDateAndTime"][0];
...
Cam.prototype._envelopeHeader = function (openHeader) {
  var header = '<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing">' + "<s:Header>";
