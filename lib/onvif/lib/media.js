...
module.exports = function (Cam) {
  ...
  Cam.prototype.getVideoSources = function (callback) {
    this._request(
      {
        service: "media",
        body: this._envelopeHeader() + '<GetVideoSources xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
    ...
    this._request(
      {
        service: "media",
        body:
          this._envelopeHeader() + '<GetVideoSourceConfigurations xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
        ...
        body:
          ...
          (options.rateControl
            ? '<RateControl xmlns="http://www.onvif.org/ver10/schema">' +
              ...
              (options.rateControl.encodingInterval
                ? "<EncodingInterval>" + options.rateControl.encodingInterval + "</EncodingInterval>"
                : "") +
          ...
          (options.multicast
            ? '<Multicast xmlns="http://www.onvif.org/ver10/schema">' +
              (options.multicast.address
                ? "<Address>" +
                  (options.multicast.address === 0
                    ? "0"
                    : (options.multicast.address.type ? "<Type>" + options.multicast.address.type + "</Type>" : "") +
                      (options.multicast.address.IPv4Address
                        ? "<IPv4Address>" + options.multicast.address.IPv4Address + "</IPv4Address>"
                        : "") +
                      (options.multicast.address.IPv6Address
                        ? "<IPv6Address>" + options.multicast.address.IPv6Address + "</IPv6Address>"
                        : "")) +
                  "</Address>"
                : "") +
          ...
          (options.sessionTimeout
            ? '<SessionTimeout xmlns="http://www.onvif.org/ver10/schema">' + options.sessionTimeout + "</SessionTimeout>"
            : "") +
          "</Configuration>" +
      ...
      function (err, data, xml) {
        if (err || linerase(data).setVideoEncoderConfigurationResponse !== "") {
          return callback.call(
            this,
            linerase(data).setVideoEncoderConfigurationResponse !== "" ? new Error("Wrong `SetVideoEncoderConfiguration` response") : err,
            data,
            xml,
          );
        }
  ...
  Cam.prototype.getAudioSources = function (callback) {
    this._request(
      {
        service: "media",
        body: this._envelopeHeader() + '<GetAudioSources xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
    ...
    this._request(
      ...
      function (err, data, xml) {
        if (err || linerase(data).setAudioEncoderConfigurationResponse !== "") {
          return callback.call(
            this,
            linerase(data).setAudioEncoderConfigurationResponse !== "" ? new Error("Wrong `SetAudioEncoderConfiguration` response") : err,
            data,
            xml,
          );
        }
    ...
    this._request(
      {
        service: "media",
        body:
          this._envelopeHeader() + '<GetAudioSourceConfigurations xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
  ...
  Cam.prototype.getAudioOutputs = function (callback) {
    this._request(
      {
        service: "media",
        body: this._envelopeHeader() + '<GetAudioOutputs xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
    ...
    this._request(
      {
        service: "media",
        body:
          this._envelopeHeader() + '<GetAudioOutputConfigurations xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
  ...
  Cam.prototype.getProfiles = function (callback) {
    if (this.media2Support) {
          ...
          if (!err) {
            ...
            this.profiles = data[0]["getProfilesResponse"][0]["profiles"].map(function (profile) {
              ...
              if (tmp.configurations.analytics) {
                newProfile.analyticsConfiguration = tmp.configurations.analytics;
              }
              if (tmp.configurations.metadata) {
                newProfile.metadataConfiguration = tmp.configurations.metadata;
              }
              if (tmp.configurations.audioOutput) {
                newProfile.audioOutputConfiguration = tmp.configurations.audioOutput;
              }
              if (tmp.configurations.audioOutput) {
                newProfile.audioDecoderConfiguration = tmp.configurations.audioDecoder;
              }
      ...
      this._request(
        {
          service: "media",
          body: this._envelopeHeader() + '<GetProfiles xmlns="http://www.onvif.org/ver10/media/wsdl"/>' + this._envelopeFooter(),
        },
        function (err, data, xml) {
          if (!err) {
            /**
  ...
  Cam.prototype.getStreamUri = function (options, callback) {
    ...
    if (this.media2Support) {
      ...
      const stream = options.stream || "RTP-Unicast";
      if (protocol == "HTTP") {
  ...
  Cam.prototype.getOSDs = function (token, callback) {
    ...
    const mediaType = this.media2Support ? "media2" : "media";
    const mediaNS = this.media2Support ? "http://www.onvif.org/ver20/media/wsdl" : "http://www.onvif.org/ver10/media/wsdl";
    this._request(
      {
        service: mediaType,
        body:
    ...
    this._request(
      {
        ...
        body:
          ...
          '" >' +
          "<ConfigurationToken>" +
          (options.videoSourceConfiguationToken || this.activeSource.videoSourceConfigurationToken) +
          "</ConfigurationToken>" +
          "</GetOSDOptions>" +
          this._envelopeFooter(),
      },
    ...
    this._request(
      {
        ...
        body:
         ...
         <wsdl:OSD token="">
            <sch:VideoSourceConfigurationToken>${
              options.videoSourceConfiguationToken || this.activeSource.videoSourceConfigurationToken
            }</sch:VideoSourceConfigurationToken>
        ...
        body:
          ...
          `<wsdl:SetOSD xmlns:wsdl="${mediaNS}" xmlns:sch="http://www.onvif.org/ver10/schema">
			 <wsdl:OSD token="${options.OSDToken}">
				<sch:VideoSourceConfigurationToken>${
          options.videoSourceConfigurationToken || this.activeSource.videoSourceConfigurationToken
        }</sch:VideoSourceConfigurationToken>
				<sch:Type>Text</sch:Type>
				<sch:Position>
  ...
  Cam.prototype.getMediaServiceCapabilities = function (callback) {
    this._request(
      {
        service: "media",
        body: this._envelopeHeader() + '<GetServiceCapabilities xmlns="http://www.onvif.org/ver10/media/wsdl" />' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (callback) {
          if (!err) {
