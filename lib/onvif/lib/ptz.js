...
module.exports = function (Cam) {
  ...
  Cam.prototype.getPresets = function (options, callback) {
    ...
    this._request(
      ...
      function (err, data, xml) {
        if (!err) {
          this.presets = {};
          let presets = linerase(data).getPresetsResponse.preset;
  ...
  Cam.prototype.getNodes = function (callback) {
    this._request(
      {
        service: "ptz",
        body: this._envelopeHeader() + '<GetNodes xmlns="http://www.onvif.org/ver20/ptz/wsdl" />' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
      ...
      function (err, data, xml) {
        if (!err) {
          ...
          data[0]["getConfigurationsResponse"].forEach(function (configuration) {
            const pTZConfiguration = configuration["PTZConfiguration"][0];
            let name = pTZConfiguration["name"];
            ...
            if ("defaultPTZSpeed" in pTZConfiguration) {
              const defaultPTZSpeed = pTZConfiguration["defaultPTZSpeed"][0];
            ...
            if ("panTiltLimits" in pTZConfiguration) {
              ...
              const panTiltLimits = pTZConfiguration["panTiltLimits"][0]["range"][0];
            ...
            if ("zoomLimits" in pTZConfiguration) {
              ...
              const zoomLimits = pTZConfiguration["zoomLimits"][0]["range"][0];
  ...
  Cam.prototype.getConfigurationOptions = function (configurationToken, callback) {
    this._request(
      ...
      function (err, data, xml) {
        let configOptions;
        if (!err) {
          const sp = data[0]["getConfigurationOptionsResponse"][0]["PTZConfigurationOptions"][0]["spaces"][0];
          configOptions = {
            ptzTimeout: {
  ...
  Cam.prototype.continuousMove = function (options, callback) {
    ...
    if (options.onlySendPanTilt) {
      delete options.zoom;
    }
    if (options.onlySendZoom) {
      delete options.x;
      delete options.y;
    }
  ...
  Cam.prototype._panTiltZoomVectors = function (ptz) {
    return ptz
      ? (ptz.x !== undefined && ptz.y !== undefined
          ...
          : "") + (ptz.zoom !== undefined ? '<Zoom x="' + ptz.zoom + '" xmlns="http://www.onvif.org/ver10/schema"/>' : "")
      : "";
  };
};
