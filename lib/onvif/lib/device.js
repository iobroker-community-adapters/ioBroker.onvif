...
module.exports = function (Cam) {
  ...
  Cam.prototype.getNTP = function (callback) {
    this._request(
      {
        service: "device",
        body: this._envelopeHeader() + '<GetNTP xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + "</s:Body>" + "</s:Envelope>",
      },
  ...
  Cam.prototype.setNTP = function (options, callback) {
    ...
    let body =
      this._envelopeHeader() + '<SetNTP xmlns="http://www.onvif.org/ver10/device/wsdl">' + "<FromDHCP>" + options.fromDHCP + "</FromDHCP>";
    ...
    body += "</SetNTP>" + this._envelopeFooter();
    this._request(
      {
        service: "device",
        body: body,
  ...
  Cam.prototype.getNetworkInterfaces = function (callback) {
    this._request(
      {
        service: "device",
        body: this._envelopeHeader() + '<GetNetworkInterfaces xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
  ...
  Cam.prototype.setNetworkInterfaces = function (options, callback) {
    ...
    const body =
      ...
      "<InterfaceToken>" +
      options.interfaceToken +
      "</InterfaceToken>" +
      "<NetworkInterface>" +
      '<Enabled xmlns="http://www.onvif.org/ver10/schema">' +
      ni.enabled +
      "</Enabled>" +
      (ni.link
        ? '<Link xmlns="http://www.onvif.org/ver10/schema">' +
          "<AutoNegotiation>" +
          ni.link.autoNegotiation +
          "</AutoNegotiation>" +
          "<Speed>" +
          ni.link.speed +
          "</Speed>" +
    ...
    this._request(
      ...
      function (err, data, xml) {
        if (callback) {
          if (!err) {
            if (modifyIpv4) {
              this.hostname = ni.IPv4.manual.address;
            }
            if (modifyIpv6) {
              this.hostname = ni.IPv6.manual.address;
            }
    ...
    this._request(
      {
        service: "device",
        body:
          this._envelopeHeader() + '<GetNetworkDefaultGateway xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (callback) {
          if (!err) {
  ...
  Cam.prototype.getDNS = function (callback) {
    this._request(
      {
        service: "device",
        body: this._envelopeHeader() + '<GetDNS xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (callback) {
          if (!err) {
  ...
  Cam.prototype.setDNS = function (options, callback) {
    ...
    body += "</SetDNS>" + this._envelopeFooter();
    this._request(
      {
        service: "device",
        body: body,
  ...
  Cam.prototype.getNetworkProtocols = function (callback) {
    this._request(
      {
        service: "device",
        body: this._envelopeHeader() + '<GetNetworkProtocols xmlns="http://www.onvif.org/ver10/device/wsdl"/>' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (!err) {
  ...
  Cam.prototype.getUsers = function (callback) {
    this._request(
      {
        service: "device",
        body: this._envelopeHeader() + '<GetUsers xmlns="http://www.onvif.org/ver10/device/wsdl" />' + this._envelopeFooter(),
      },
      function (err, data, xml) {
        if (callback) {
          if (!err) {
  ...
  Cam.prototype.createUsers = function (users, callback) {
    ...
    users.forEach(function (user) {
      usersXml +=
        "<User>" +
        '<Username xmlns="http://www.onvif.org/ver10/schema">' +
        user.username +
        "</Username>" +
        '<Password xmlns="http://www.onvif.org/ver10/schema">' +
        user.password +
        "</Password>" +
        '<UserLevel xmlns="http://www.onvif.org/ver10/schema">' +
        user.userLevel +
        "</UserLevel>" +
        "</User>";
    });
      ...
      function (err, data, xml) {
        if (callback) {
          if (err) {
            ...
            if (delayGet) {
              // On some cameras, changing the user password we currently use is not effective right away
              setTimeout(
                function () {
                  this.getUsers(callback);
                }.bind(this),
